...
namespace PaymentPurchaseManagementService.Service.GenericServices.Static
{
    public class MailerService
    {
            ...
            [CallerMemberName] string caller = "")
        {
            ...
            {
                await _loggingService.LogError("To address is missing.", caller, new ArgumentNullException(nameof(to)), Guid.NewGuid().ToString());
            ...
            try
            {
                ...
                await _loggingService.LogInformation($"Email sent successfully to {to}.", caller,Guid.NewGuid().ToString());
                return true;
            }
            catch (Exception ex)
            {
                await _loggingService.LogFatal("An error occurred while sending the email.", caller, ex, Guid.NewGuid().ToString());
